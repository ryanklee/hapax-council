...
SHARED="$SHARED,backlight_compensation=0,brightness=128,contrast=135,saturation=128"

# --- BRIO (hero/operator) — 1080p, larger sensor, low gain ---
DEV=/dev/v4l/by-id/usb-046d_Logitech_BRIO_5342C819-video-index0
if [ -e "$DEV" ]; then
  $V4L2 -d "$DEV" --set-ctrl="$SHARED,gain=80,exposure_time_absolute=250,sharpness=140"
  $V4L2 -d "$DEV" --set-ctrl=focus_automatic_continuous=0,focus_absolute=0
  echo "brio-operator: configured"
fi
...
if [ -e "$DEV" ]; then
  $V4L2 -d "$DEV" --set-ctrl="$SHARED,gain=180,exposure_time_absolute=250,sharpness=150"
  $V4L2 -d "$DEV" --set-ctrl=focus_automatic_continuous=0,focus_absolute=0
  echo "c920-hardware: configured"
fi

# --- C920-room (wide room view) ---
DEV=/dev/v4l/by-id/usb-046d_HD_Pro_Webcam_C920_86B6B75F-video-index0
if [ -e "$DEV" ]; then
  $V4L2 -d "$DEV" --set-ctrl="$SHARED,gain=200,exposure_time_absolute=250,sharpness=150"
  $V4L2 -d "$DEV" --set-ctrl=focus_automatic_continuous=0,focus_absolute=30
  echo "c920-room: configured"
fi
