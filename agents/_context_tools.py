"""shared/context_tools.py — On-demand operator context tools for Pydantic AI agents.

Provides reusable tool functions that any agent can register to access
operator constraints, patterns, and profile data on demand — replacing
...
async def lookup_constraints(ctx: RunContext[Any], categories: str = "") -> str:
    ...
    from agents._operator import get_constraints
...
async def lookup_patterns(ctx: RunContext[Any], categories: str = "") -> str:
    ...
    from agents._operator import get_patterns
...
async def search_profile(ctx: RunContext[Any], query: str, dimension: str = "") -> str:
    ...
    Returns the most relevant profile facts matching the query. Use this to
    understand the operator's preferences, skills, and patterns.

    Args:
        query: Natural language search query (e.g., "preferred Python tools").
        dimension: Optional dimension filter (e.g., "workflow", "identity").
    """
...
async def get_profile_summary(ctx: RunContext[Any], dimension: str = "") -> str:
    ...
    Returns a narrative summary of the operator's profile. Call without arguments
    for the overall summary, or specify a dimension for detail.

    Args:
        dimension: Profile dimension name (e.g., "workflow", "identity"). Empty for overall.
    """
    ...
    if not digest:
        return "No profile digest available. Run `profiler --digest` to generate one."
