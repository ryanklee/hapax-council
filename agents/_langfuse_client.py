"""Vendored Langfuse API client for the agents package.

Provides authenticated HTTP access to Langfuse's public API.
"""
...
log = logging.getLogger("agents._langfuse_client")
...
def langfuse_get(path: str, params: dict | None = None, *, timeout: int = 15) -> dict:
    ...
    if not LANGFUSE_PK or not LANGFUSE_SK:
        log.debug("langfuse: no credentials configured")
        return {}
...
def query_zero_result_spans(hours: int = 24) -> list[dict]:
    """Find RAG spans with zero results in the last N hours."""
    if not LANGFUSE_PK:
        return []
...
def query_recent_errors(service: str, hours: int = 1) -> list[dict]:
    """Find error-level traces for a service in the last N hours."""
    if not LANGFUSE_PK:
        return []
