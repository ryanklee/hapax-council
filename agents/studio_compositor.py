...
class StudioCompositor:
    ...
    def _add_smooth_delay_branch(self, pipeline: Any, tee: Any) -> None:
        """Add smooth delay branch — @smooth layer source.

        Pipeline: tee → queue → convert(RGBA) → glupload → glcolorconvert →
                  smoothdelay → glcolorconvert → gldownload → convert →
                  videoscale → videorate → jpegenc → appsink (smooth-snapshot.jpg)
        """
        Gst = self._Gst
