...
async def get_flow_state(request: Request) -> dict:
    ...
    if _observer is None:
        _event_bus = getattr(request.app.state, "event_bus", None)
        _observer = FlowObserver(event_bus=_event_bus)
        for node in nodes:
    ...
    # Append external nodes from event bus (LLM, Qdrant, Pi Fleet)
    bus = getattr(request.app.state, "event_bus", None)
    if bus:
        from logos.api.flow_external import build_external_nodes

        ext_nodes, ext_edges = build_external_nodes(bus)
