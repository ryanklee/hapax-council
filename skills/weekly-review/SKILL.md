...
2. **Health history**: `cd ~/projects/hapax-council && uv run python -m agents.health_monitor --history`
3. **Drift report** (if recent): `cat ~/projects/hapax-council/profiles/drift-report.json 2>/dev/null | jq '.drift_items | length'`
4. **Scout report** (if recent): `cat ~/projects/hapax-council/profiles/scout-report.json 2>/dev/null | jq '.evaluations | length'`
5. **Briefing** (latest): `head -30 ~/projects/hapax-council/profiles/briefing.md`
