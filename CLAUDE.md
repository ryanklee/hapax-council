...
**Consent gate:** Capabilities declaring `OperationalProperties.consent_required=True` are filtered out of `AffordancePipeline.select()` when no active consent contract exists in `axioms/contracts/`. Fail-closed; state cached 60s. Axiom `interpersonal_transparency` mandates this gate.
