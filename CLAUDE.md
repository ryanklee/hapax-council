...
**Daimonion dispatch split (PR #555 regression, beta followup):** The daimonion spawns **two** background tasks that read ``/dev/shm/hapax-dmn/impingements.jsonl`` through independent ``ImpingementConsumer`` instances, each with its own ``cursor_path``. **CPAL loop** (``_cpal_impingement_loop`` closure in ``run_inner.py``) owns gain/error modulation and spontaneous-speech surfacing — the adapter's ``should_surface`` gate calls ``pipeline.generate_spontaneous_speech`` through ``CpalRunner.process_impingement``. **Affordance loop** (``run_loops_aux.impingement_consumer_loop``) owns everything else recruited by the pipeline: ``system.notify_operator`` → ``activate_notification``, ``studio.*`` and world-domain Thompson outcome recording, ``system_awareness.activate``, ``capability_discovery.search`` chain, and ``ExpressionCoordinator.coordinate`` cross-modal dispatch (textual + notification modalities only; auditory is owned by CPAL). The loop explicitly skips ``speech_production`` candidates to avoid double-firing with CPAL. Cursor files: ``impingement-cursor-daimonion-cpal.txt`` for CPAL, ``impingement-cursor-daimonion-affordance.txt`` for affordance dispatch. Apperception cascade is NOT handled in either loop — ``shared.apperception_tick.ApperceptionTick`` inside the visual layer aggregator owns it. **Background:** PR #555 ("CPAL as sole coordinator") silently removed the affordance loop spawn while claiming the CPAL adapter "Replaces ... impingement_consumer_loop routing" — incorrect, because the adapter only transforms impingement → gain/error. Six dispatch effects were silently dead for ~10 days. Regression pin: ``tests/hapax_daimonion/test_impingement_consumer_loop.py::TestSpawnRegressionPin``.
...
- `shared/compositor_model.py` — `SourceSchema`/`SurfaceSchema`/`Assignment`/`Layout` pydantic models with difflib "did you mean" hints in validation errors. `SurfaceKind` includes `fx_chain_input` for main-layer appsrc pads (source-registry epic, PR #711).
- `agents/studio_compositor/layout_state.py` — `LayoutState` in-memory authority for the current `Layout`. RLock-guarded `get()`/`mutate(fn)`/`subscribe(cb)` with pydantic re-validation and self-write detection for reload-loop prevention. Source-registry epic, PR #711. **Dormant until Phase D wires it into `StudioCompositor.start()`.**
- `agents/studio_compositor/source_registry.py` — `SourceRegistry` maps `source_id` → backend handle (`CairoSourceRunner` or `ShmRgbaReader`). `construct_backend(source)` dispatches on `SourceSchema.backend`: `"cairo"` looks up `params.class_name` via `cairo_sources`, `"shm_rgba"` resolves to `ShmRgbaReader(params.shm_path)`. Typed error hierarchy (`UnknownSourceError`, `UnknownBackendError`). Source-registry epic, PR #711. **Dormant until Phase D.**
- `agents/studio_compositor/shm_rgba_reader.py` — `ShmRgbaReader` for `external_rgba` sources. mmaps an RGBA buffer at a shm path, reads a sidecar JSON (`{w, h, stride, frame_id}`) for frame geometry, caches by `frame_id`, and exposes `get_current_surface() -> cairo.ImageSurface | None`. Missing/malformed/short-buffer resolve to `None` without raising. This is how reverie will plug into the compositor in Phase F (reverie headless mode writes to `/dev/shm/hapax-sources/reverie.rgba` + sidecar; compositor reads via this adapter). Source-registry epic, PR #711. **Dormant until Phase D + F wire the reader into the main pipeline.**
- `agents/studio_compositor/cairo_sources/__init__.py` — `class_name` → `type[CairoSource]` registry. Three built-in registrations (`TokenPoleCairoSource`, `AlbumOverlayCairoSource`, `SierpinskiCairoSource`) re-exported from their legacy modules. `register()` is idempotent on same-class, raises on same-name/different-class (silent-failure discipline). Adding a new cairo source to a Layout is a config-only change once natural-size migration is complete. Source-registry epic, PR #711.
- `agents/studio_compositor/cairo_source.py` — `CairoSourceRunner.__init__` gains optional `natural_w`/`natural_h` kwargs separating render resolution from canvas dimensions. Defaults preserve backward compat (legacy callers that use `set_canvas_size()` still get their output surfaces resized via a `_natural_explicit` flag). Source-registry epic, PR #711. Phase C tasks 8–11 of the source-registry plan migrate the three cairo source classes to draw at local origin; **task 8 (TokenPole) deferred** because the legacy `TokenPole.draw(cr)` facade path depends on the current `OVERLAY_X=20/Y=20/SIZE=300` offsets and needs visual verification before refactoring.

**Spec + audit:**
- `docs/superpowers/plans/2026-04-12-compositor-unification-epic.md` — full epic plan
- `docs/superpowers/specs/2026-04-12-compositor-source-registry-foundation-design.md` — source-registry epic spec: make the `Layout`/`SourceSchema`/`SurfaceSchema`/`Assignment` framework authoritative, register reverie as an `external_rgba` source, migrate the three cairo overlays to natural-size + layout-driven placement, support mid-stream PiP geometry mutation via `compositor.surface.set_geometry`, lay `appsrc` pads for every source so preset chains can reference any of them. Merged in PR #709.
- `docs/superpowers/plans/2026-04-12-compositor-source-registry-foundation-plan.md` — the 29-task implementation plan for the source-registry epic. Phase A+B+C7 landed in PR #711 (7 commits, 56 new tests). **Phase C8–11 and D–J remain** — see the delta handoff for pickup guidance.
- `docs/superpowers/handoff/2026-04-12-delta-source-registry-handoff.md` — delta session handoff for a future implementer continuing the source-registry epic. Status snapshot, what's merged, what's deferred, composition with alpha's camera epic, branching convention, risks.
...
*Content layer custom[0] routing (F8 resolved).* `content_layer.wgsl` has no `@group(2) Params` binding — it composites 4 content slots and reads `material_id` / slot salience / intensity from `uniforms.custom[0]`. The fix takes option (b): `dynamic_pipeline.rs render()` routes `content.material` / `content.salience` / `content.intensity` keys from uniforms.json into `UniformData.custom[0][0..2]` on each frame, next to the existing `signal.*` override branch. The Python mixer writes all three keys from imagination fragments (`agents/reverie/_uniforms.write_uniforms`); material maps water/fire/earth/air/void → 0..4 via `MATERIAL_MAP`. Before F8 landed, material_id was effectively hardcoded to water (0) at the GPU and Bachelard Amendment 3 (material quality as shader uniform) was unreachable at runtime. Regression pin: `tests/test_reverie_uniforms_plan_schema.py::test_write_uniforms_end_to_end_produces_expected_keys` asserts all three keys land with the expected values.
