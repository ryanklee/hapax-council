...
**Visual surface (Hapax Reverie):** A standalone binary (`hapax-imagination`) runs as a systemd user service, rendering dynamic shader graphs via wgpu. Python compiles effect presets (`agents/effect_graph/wgsl_compiler.py`) into WGSL execution plans that the Rust `DynamicPipeline` hot-reloads from `/dev/shm/hapax-imagination/pipeline/`. Frames are written to `/dev/shm/hapax-visual/frame.jpg` via turbojpeg. The `VisualSurface` React component fetches frames at 30fps from the HTTP frame server (`:8053`). Tauri communicates with the binary via UDS (`$XDG_RUNTIME_DIR/hapax-imagination.sock`) for window management commands. The winit window is independently positioned (multi-monitor, fullscreen, borderless, always-on-top).
