...
def test_blocks_ambient_music() -> None:
    ...
    with patch.object(gate, "_get_sink_volume", return_value=0.3), \
         patch.object(gate, "_check_studio", return_value=(True, "")), \
         patch.object(gate, "_check_ambient", return_value=(False, "Ambient audio blocked: Music (0.85), total block prob=0.90")):
        result = gate.check()
    assert not result.eligible
...
def test_blocks_ambient_speech() -> None:
    ...
    with patch.object(gate, "_get_sink_volume", return_value=0.3), \
         patch.object(gate, "_check_studio", return_value=(True, "")), \
         patch.object(gate, "_check_ambient", return_value=(False, "Ambient audio blocked: Speech (0.72), total block prob=0.75")):
        result = gate.check()
    assert not result.eligible
...
def test_ambient_fail_closed() -> None:
    ...
    with patch.object(gate, "_get_sink_volume", return_value=0.3), \
         patch.object(gate, "_check_studio", return_value=(True, "")), \
         patch.object(gate, "_check_ambient", return_value=(False, "Ambient classification unavailable (fail-closed)")):
        result = gate.check()
    assert not result.eligible
