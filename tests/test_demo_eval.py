...
class TestEvaluateDemoOutput:
    ...
    async def test_combines_all_categories(self, tmp_path):
        ...
        with patch("agents.demo_pipeline.eval_rubrics.run_structural_checks", return_value=struct_dims), \
             patch("agents.demo_pipeline.eval_rubrics.run_text_evaluation", new_callable=AsyncMock, return_value=text_dims), \
             patch("agents.demo_pipeline.eval_rubrics.run_visual_evaluation", new_callable=AsyncMock, return_value=visual_dims), \
...
class TestRunEvalLoop:
    ...
    async def test_passes_on_first_iteration(self, tmp_path):
        ...
        with patch("agents.demo.generate_demo", new_callable=AsyncMock, return_value=fake_demo_dir), \
             patch("agents.demo_eval.evaluate_demo_output", new_callable=AsyncMock, return_value=passing_report), \
             patch("agents.demo.parse_request", return_value=("the system", "my wife")), \
    ...
    async def test_heals_on_second_iteration(self, tmp_path):
        ...
        with patch("agents.demo.generate_demo", new_callable=AsyncMock, return_value=fake_demo_dir), \
             ...
             patch("agents.demo_pipeline.eval_rubrics.diagnose_failures", new_callable=AsyncMock, return_value=mock_diagnosis), \
             patch("agents.demo.parse_request", return_value=("the system", "my wife")), \
             patch("agents.demo.resolve_audience", return_value=("family", "")), \
             patch("agents.demo_models.load_personas", return_value={"family": MagicMock()}), \
             patch("agents.demo.parse_duration", return_value=180), \
             ...
             patch("agents.demo_pipeline.lessons.extract_lessons", return_value=["Use simple language"]), \
