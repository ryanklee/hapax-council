...
def test_all_sync_agent_facts_target_behavioral_dimensions():
    ...
    from agents.gcalendar_sync import _generate_profile_facts as gcal_facts, CalendarSyncState, CalendarEvent
    from agents.gmail_sync import _generate_profile_facts as gmail_facts, GmailSyncState, EmailMetadata
    from agents.chrome_sync import _generate_profile_facts as chrome_facts, ChromeSyncState
    from agents.youtube_sync import _generate_profile_facts as yt_facts, YouTubeSyncState, LikedVideo, Subscription
    from agents.obsidian_sync import _generate_profile_facts as obs_facts, ObsidianSyncState, VaultNote
    from agents.claude_code_sync import _generate_profile_facts as cc_facts, ClaudeCodeSyncState, TranscriptMetadata
    from agents.audio_processor import _generate_profile_facts as audio_facts, AudioProcessorState, ProcessedFileInfo

    # Build minimal states that produce facts
    gcal_state = CalendarSyncState(events={
        "1": CalendarEvent(event_id="1", summary="standup", start="2026-03-10T09:00:00Z",
                           end="2026-03-10T09:30:00Z", attendees=["a@b.com"], recurring=True),
    })
    gmail_state = GmailSyncState(messages={
        "1": EmailMetadata(message_id="1", thread_id="t1", sender="x@y.com",
                           subject="hi", timestamp="2026-03-10T10:00:00Z", labels=["INBOX"]),
    })
    chrome_state = ChromeSyncState(domains={"github.com": 50})
    yt_state = YouTubeSyncState(
        liked_videos={"v1": LikedVideo(video_id="v1", title="t", channel="ch",
                                        tags=["music"], published="2026-01-01")},
        subscriptions={"ch1": Subscription(channel_id="ch1", channel_name="Chan")},
    )
    obs_state = ObsidianSyncState(notes={
        "n1": VaultNote(relative_path="folder/note.md", title="note", folder="folder",
                        content_hash="abc123", size=100, mtime=1710000000.0, tags=["tag"]),
    })
    cc_state = ClaudeCodeSyncState(sessions={
        "s1": TranscriptMetadata(session_id="s1", project_name="proj", message_count=10),
    })
    audio_state = AudioProcessorState(processed_files={
        "f1": ProcessedFileInfo(filename="rec.wav", speech_seconds=100, music_seconds=50,
                                silence_seconds=20, segment_count=5, speaker_count=2),
    })
