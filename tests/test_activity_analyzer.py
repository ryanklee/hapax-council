...
def test_collect_langfuse_with_errors(mock_api):
    ...
    mock_api.side_effect = lambda path, params=None: (
        traces_resp if "/traces" in path else obs_resp
    )
...
def test_collect_health_trend_with_data(tmp_path):
    ...
    entries = [
        {"timestamp": (now - timedelta(hours=2)).isoformat(), "status": "healthy",
         "healthy": 40, "degraded": 0, "failed": 0, "duration_ms": 2000, "failed_checks": []},
        {"timestamp": (now - timedelta(hours=1)).isoformat(), "status": "degraded",
         "healthy": 38, "degraded": 2, "failed": 0, "duration_ms": 2100,
         "failed_checks": ["docker.n8n", "docker.langfuse"]},
        {"timestamp": (now - timedelta(minutes=30)).isoformat(), "status": "healthy",
         "healthy": 40, "degraded": 0, "failed": 0, "duration_ms": 1900, "failed_checks": []},
    ]
    ...
    assert result.uptime_pct == pytest.approx(66.7, abs=0.1)
...
def test_collect_health_trend_filters_by_time(tmp_path):
    ...
    entries = [
        {"timestamp": (now - timedelta(hours=48)).isoformat(), "status": "failed",
         "healthy": 30, "degraded": 0, "failed": 10, "duration_ms": 3000, "failed_checks": ["x"]},
        {"timestamp": (now - timedelta(hours=1)).isoformat(), "status": "healthy",
         "healthy": 40, "degraded": 0, "failed": 0, "duration_ms": 2000, "failed_checks": []},
    ]
    history_file.write_text("\n".join(json.dumps(e) for e in entries))
...
def test_collect_drift_trend_no_files(tmp_path):
    with patch("agents.activity_analyzer.DRIFT_REPORT", tmp_path / "nope.json"), \
...
def test_collect_drift_trend_with_report(tmp_path):
    ...
    with patch("agents.activity_analyzer.DRIFT_REPORT", report_file), \
...
def test_format_human_with_langfuse_data():
    report = ActivityReport(
        ...
        langfuse=LangfuseActivity(
            ...
            total_cost=0.05,
            models=[
                ModelUsage(model_group="claude-haiku", call_count=10,
                           total_input_tokens=3000, total_output_tokens=1000,
                           total_cost=0.03, avg_latency_ms=200),
                ModelUsage(model_group="qwen-7b", call_count=5,
                           total_input_tokens=2000, total_output_tokens=1000,
                           total_cost=0.02, avg_latency_ms=100),
            ],
        ),
    )
    output = format_human(report)
...
def test_format_human_with_health_data():
    report = ActivityReport(
        ...
        hours=12,
        health=HealthTrend(
            total_runs=10, healthy_runs=8, degraded_runs=2,
            uptime_pct=80.0, avg_duration_ms=2000,
...
def test_format_human_with_service_events():
    report = ActivityReport(
        ...
        hours=12,
        service_events=[
            ServiceEvent(unit="health-monitor.service", event="completed",
...
def test_format_human_with_errors():
    report = ActivityReport(
        ...
        hours=12,
        langfuse=LangfuseActivity(
            total_traces=5, total_generations=5, error_count=2,
