...
class TestPipelineIntegration:
    ...
    @pytest.mark.asyncio
    async def test_perception_sync_called_each_tick(self):
        sync_fn = MagicMock()
        loop = _make_loop(perception_sync_fn=sync_fn)
        task = asyncio.create_task(loop.run())
    ...
    async def test_model_updated_on_utterance_dispatch(self):
        model = ConversationalModel()
        buf = _mock_buffer()
        buf.get_utterance.return_value = b"\x00" * 32000  # 1s audio
        pipeline = _mock_pipeline(state="transcribing")
