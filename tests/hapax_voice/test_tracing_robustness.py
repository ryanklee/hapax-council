...
def test_missing_public_key():
    ...
    with patch.dict("os.environ", {
        ...
        with patch("agents.hapax_voice.tracing.Langfuse", MagicMock()):
...
def test_trace_exception_during_trace_call():
    ...
    with patch.dict("os.environ", {
        ...
        with patch("agents.hapax_voice.tracing.Langfuse", return_value=mock_client):
            tracer = VoiceTracer(enabled=True)
...
def test_context_manager_exception_in_body():
    ...
    with tracer.trace_analysis(presence_score="unknown") as span:
...
def test_disabled_tracer_never_creates_client():
    ...
    with patch("agents.hapax_voice.tracing.Langfuse") as mock_cls:
        ...
        with tracer.trace_analysis(presence_score="away") as span:
            pass
        with tracer.trace_session(session_id="s1", trigger="hotkey") as span:
            pass
        with tracer.trace_delivery(
            session_id=None, presence_score="away",
            gate_reason="none", notification_priority="low",
        ) as span:
            pass
