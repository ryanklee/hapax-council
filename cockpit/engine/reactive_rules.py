...
# ── Phase 2: Drift reconciliation (cloud LLM) ────────────────────────────


# Track reconciliation runs per timer period to enforce cycle cap
_drift_reconcile_runs: list[float] = []
_DRIFT_RECONCILE_MAX_PER_PERIOD = 3
_DRIFT_RECONCILE_PERIOD_S = 43200  # 12 hours (one timer period)


async def _handle_drift_reconcile(*, ignore_fn=None) -> str:
    """Run reactive drift reconciliation from existing report."""
    import json
    ...
    if ignore_fn is not None:
        ignore_fn(PROFILES_DIR / "drift-report.json")
        ignore_fn(PROFILES_DIR / "drift-remediation.jsonl")
        ignore_fn(PROFILES_DIR / "drift-metrics.json")

    from agents.drift_detector import DriftReport, reconcile
    ...
    result = await reconcile(from_report=from_report)
    _log.info(
        "Reactive drift reconciliation complete: %d items remaining",
        len(result.drift_items),
    )
    return f"reconciled:{len(result.drift_items)}-remaining"


def _drift_reconcile_filter(event: ChangeEvent) -> bool:
    """Match drift-report.json writes that contain auto-fixable items."""
    if event.path.name != "drift-report.json":
        return False

    # Enforce cycle cap: max N runs per timer period
    now = time.monotonic()
    # Prune old entries
    cutoff = now - _DRIFT_RECONCILE_PERIOD_S
    while _drift_reconcile_runs and _drift_reconcile_runs[0] < cutoff:
        _drift_reconcile_runs.pop(0)

    if len(_drift_reconcile_runs) >= _DRIFT_RECONCILE_MAX_PER_PERIOD:
        _log.debug("Drift reconcile cycle cap reached (%d runs)", len(_drift_reconcile_runs))
        return False

    # Check if report contains auto-fixable items
    try:
        import json

        data = json.loads(event.path.read_text())
        items = data.get("drift_items", [])
        has_auto_fix = any(
            i.get("remediability") == "auto_fix" and float(i.get("confidence", 0)) >= 0.9
            for i in items
        )
        if has_auto_fix:
            _drift_reconcile_runs.append(now)
            return True
    except Exception:
        pass
    return False
...
def _drift_reconcile_produce(event: ChangeEvent) -> list[Action]:
    return [
        Action(
            name="drift-reconcile-reactive",
            handler=_handle_drift_reconcile,
            args={},
            phase=2,
            priority=70,
        )
...
DRIFT_RECONCILE_RULE = Rule(
    name="drift-reconcile",
    description="Reactive drift reconciliation when report contains auto-fixable items",
    trigger_filter=_drift_reconcile_filter,
    produce=_drift_reconcile_produce,
    phase=2,
    cooldown_s=60,  # 60-second debounce
)
